...
minmpt_error minmpt_eval_logits(minmpt_handle handle, const uint32_t *tokens,
  ...
  if (modelp->mem_per_token == 0) {
    ...
    mpt_eval_cpp(modelp->model, modelp->n_threads, 0, {1, 2, 3, 4},
                 dummy_logits, modelp->mem_per_token);
  }
  if (!mpt_eval(modelp->model, modelp->n_threads, modelp->n_past, tokens,
                n_tokens, logits, modelp->mem_per_token)) {
