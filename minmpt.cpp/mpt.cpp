...
enum minmpt_format_version
{
    minmpt_format_v1_no_vocab = 0
};
...
bool mpt_model_load(const std::string &fname, mpt_model &model, size_t n_ctx_override)
{
    ...
    // load weights
    {
        ...
        while (mptf.tell() < mptf.size)
        {
            ...
            mptf.read_raw(reinterpret_cast<char *>(tensor->data), ggml_nbytes(tensor));
    ...
    size_t &mem_per_token)
{
    ...
    memcpy(embd_w, (float *)ggml_get_data(out) + (n_vocab * (N - 1)), sizeof(float) * n_vocab);
    ...
    size_t &mem_per_token)
{
    ...
    return mpt_eval(model, n_threads, n_past, embd_inp.data(), embd_inp.size(), embd_w.data(), mem_per_token);
}